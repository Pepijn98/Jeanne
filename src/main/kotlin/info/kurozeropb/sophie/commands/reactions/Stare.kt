        ...
        Utils.catchAll("Exception occured in ${this.name} command", e.channel) {
            Sophie.weebApi.imageProvider.getRandomImage(this.name, HiddenMode.DEFAULT, NsfwFilter.NO_NSFW).async({ image ->
                ...
                if (resp.isSuccessful) {
                    val body = resp.body()
                    if (body != null)
                        e.reply(body.byteStream(), "${image.id}.${image.fileType.name.toLowerCase()}", if (member != null) "**${e.member.effectiveName}** stares at **${member.effectiveName}**" else null)
                    else
