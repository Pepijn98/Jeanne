...
import dev.vdbroek.jeanne.Guild as dbGuild
import dev.vdbroek.jeanne.User as dbUser
    ...
    companion object {
        ...
        fun initialize(config: Config) {
            ...
            val milli = measureTimeMillis {
                client =
                    KMongo.createClient(ConnectionString("mongodb://${config.database.user}:${config.database.passwd}@${config.database.host}:${config.database.port}/${config.database.name}?retryWrites=true"))
