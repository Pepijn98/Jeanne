package dev.vdbroek.jeanne.commands
...
abstract class Command(
    ...
    fun MessageReceivedEvent.reply(embed: MessageEmbed, success: Consumer<Message>? = null) = Utils(this).reply(embed, success)

    fun MessageReceivedEvent.reply(text: String, success: Consumer<Message>? = null) = Utils(this).reply(text, success)

    fun MessageReceivedEvent.reply(data: InputStream, fileName: String, message: String? = null, success: Consumer<Message>? = null) = Utils(this).reply(data, fileName, message, success)

    fun MessageReceivedEvent.reply(file: File, fileName: String, message: String? = null, success: Consumer<Message>? = null) = Utils(this).reply(file, fileName, message, success)

    fun MessageReceivedEvent.reply(bytes: ByteArray, fileName: String, message: String? = null, success: Consumer<Message>? = null) = Utils(this).reply(bytes, fileName, message, success)

    fun asData(): CommandData = CommandData(
        name = this.name,
        ...
        isDonatorsOnly = this.isDonatorsOnly,
        allowPrivate = this.allowPrivate,
        isDeveloperOnly = this.isDeveloperOnly,
