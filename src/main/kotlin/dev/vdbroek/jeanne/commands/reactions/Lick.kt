...
class Lick : Command(
    ...
    description = "Lick someone? Weird flex but ok.",
    allowPrivate = false,
        ...
        Utils.catchAll("Exception occured in ${this.name} command", e.channel) {
            Jeanne.weebApi.imageProvider.getRandomImage(this.name, HiddenMode.DEFAULT, NsfwFilter.NO_NSFW).async({ image ->
                ...
                if (resp.isSuccessful) {
                    val body = resp.body()
                    if (body != null)
                        e.reply(body.byteStream(), "${image.id}.${image.fileType.name.toLowerCase()}", "**${e.member?.effectiveName}** licks **${member.effectiveName}**")
                    else
